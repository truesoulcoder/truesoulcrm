...
gmail_monitor_logger.propagate = False
...
if supabase_client:
    supabase_handler_gm = SupabaseLogHandler(
        ...
        script_name_override='gmail_event_monitor.py'
    )
    supabase_handler_gm.setLevel(supabase_log_level_for_handler)
    supabase_handler_gm.setFormatter(console_formatter_gm)
...
def is_bounce(message):
    try:
        ...
        if body:
            ...
            if any(phrase in body for phrase in BOUNCE_PHRASES):
                return True
    ...
    try:
        try:
            ...
            if 'SPAM' in msg.get('labelIds', []):
                gmail_monitor_logger.debug(f"Skipping spam message {message_id}")
                return
                
        except HttpError as e:
            if e.resp.status == 404:
                gmail_monitor_logger.debug(f"Message {message_id} not found (may be permanently deleted)")
                return
            raise
        ...
        if 'event_type' in event_details:
            try:
                db_event = {
                    ...
                    'contact_email': event_details.get('contact_email') or sender_email_used,
                    'event_type': event_details['event_type'],
                    'event_timestamp': event_details['timestamp'],
                    ...
                    'raw_event_data': {'original_data': event_details}
                }
                
                gmail_monitor_logger.debug(f"Attempting to insert engagement event: {db_event}")
                result = db_client.table("email_engagement_events").insert(db_event).execute()
    ...
    for sender in senders_res.data:
        ...
        try:
            if not last_history_id:
                ...
                if current_sender_history_id:
                    ...
                    try:
                        ...
                        last_history_id = str(current_sender_history_id)
            ...
            while True:
                ...
                current_page_history_id = response.get('historyId', last_history_id)

                if 'history' in response:
                    for history_record in response['history']:
                        if 'messagesAdded' in history_record:
                            for msg_added_record in history_record['messagesAdded']:
                                message_obj = msg_added_record.get('message', {})
                ...
                if not page_token:
                    if current_page_history_id and (not last_history_id or int(current_page_history_id) > int(last_history_id)):
                        try:
                            ...
                            last_history_id = str(current_page_history_id)
                        except Exception as db_e:
                            logger = logging.getLogger('gmail_event_monitor')
