// Top of file
console.log("[set-truesoul-role] Function triggered");

// Inside handleUserCreated
console.log(`[DEBUG] Processing user: ${userEmail}`);
...
serve(async (req)=>{
  ...
  try {
    ...
    if (payload.type !== 'INSERT' || payload.table !== 'users' || !userRecord) {
      ...
      return new Response(JSON.stringify({
        message: 'Irrelevant event, skipping role assignment.'
      }), {
        status: 200,
        headers: {
          'Content-Type': 'application/json'
    ...
    if (!userRecord.email) {
      ...
      return new Response(JSON.stringify({
        error: 'User record or email missing'
      }), {
        status: 400,
        headers: {
          'Content-Type': 'application/json'
    ...
    let newRole = userRecord.raw_app_meta_data?.role || null; // Start with existing role or null
    ...
    if (SUPERADMIN_EMAILS.includes(userEmail)) {
      newRole = 'superadmin';
      roleChanged = true;
      console.log(`[set-truesoul-role] Assigning superadmin role to ${userEmail}`);
    } else if (userEmail.endsWith(expectedDomain)) {
    ...
    if (roleChanged && newRole) {
      ...
      const { data, error: updateError } = await supabaseAdmin.auth.admin.updateUserById(userRecord.id, {
        app_metadata: {
          ...userRecord.raw_app_meta_data,
          role: newRole
        }
      });
      if (updateError) {
    ...
    return new Response(JSON.stringify({
      message: `Role processing complete for ${userEmail}. Role: ${newRole || 'unchanged'}`
    }), {
      status: 200,
      headers: {
        'Content-Type': 'application/json'
