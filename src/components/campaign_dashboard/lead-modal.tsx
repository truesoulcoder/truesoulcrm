  ...
  return (
    <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl" backdrop="blur">
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader className="flex flex-col gap-1">{isNew ? "Add New Lead" : "Edit Lead"}</ModalHeader>
            <ModalBody>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input autoFocus label="Name" value={formData.name || ''} onValueChange={(v) => handleChange("name", v)} isRequired />
                <Input type="email" label="Email" value={formData.email || ''} onValueChange={(v) => handleChange("email", v)} isRequired />
                <Input label="Phone" value={formData.phone || ''} onValueChange={(v) => handleChange("phone", v)} />
                <Input label="Company" value={formData.company || ''} onValueChange={(v) => handleChange("company", v)} />
                <Select label="Status" selectedKeys={formData.status ? [formData.status] : []} onChange={(e) => handleChange("status", e.target.value)}>
                  {statusOptions.map((s) => (<SelectItem key={s.key}>{s.label}</SelectItem>))}
                </Select>
                <Select label="Source" selectedKeys={formData.source ? [formData.source] : []} onChange={(e) => handleChange("source", e.target.value)}>
                  {sourceOptions.map((s) => (<SelectItem key={s.key}>{s.label}</SelectItem>))}
                </Select>
              </div>
            </ModalBody>
            <ModalFooter>
              <div className="flex w-full justify-between">
                {!isNew && (
                  <Button color="danger" variant="light" onPress={handleDelete} startContent={<Icon icon="lucide:trash-2" />}>
                    Delete
                  </Button>
                )}
