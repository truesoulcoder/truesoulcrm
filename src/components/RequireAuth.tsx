...
export default function RequireAuth({ children }: { children: ReactNode }) { //
  ...
  useEffect(() => { //
    ...
    if (!user) { //
      // If the user is not logged in and not on a public path, redirect to the root page.
      if (!isPublicPath) { //
        targetPath = '/'; // FIX: Changed from '/login' to '/'
      } //
    } else { //
      // User is logged in.
      // If they are on a public path (like the root/login page), redirect them to their dashboard.
      if (isPublicPath) { //
